...
function collectBuildSteps(node: BomNode): BomNode[] {
  const result: BomNode[] = [];
  if (node.decision === "build" || node.decision === "build_override") {
    result.push(node);
  }
  for (const child of node.children ?? []) {
    result.push(...collectBuildSteps(child));
  }
  return result;
}
...
function WarehousePanel({
  ...
  const buildSteps = useMemo<BomNode[]>(() => {
    if (!bom) return [];
    const steps: BomNode[] = [bom];
    for (const child of bom.children ?? []) {
      steps.push(...collectBuildSteps(child));
    }
    return steps;
  }, [bom]);

  const totalJobCost = buildSteps.reduce((s, n) => s + (n.job_cost ?? 0), 0);
    ...
    <div className="border-t border-overlay-subtle bg-background-panel h-[300px] flex">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <Tabs defaultValue="shopping" className="flex flex-col h-full">
          ...
          <TabsContent value="jobcosts" className="flex-1 overflow-y-auto m-0 p-0">
            {buildSteps.length > 0 ? (
              <>
                <div className="flex items-center justify-between px-3 py-1.5 border-b border-overlay-subtle bg-background-elevated">
                  <span className="text-xs text-text-muted">
                  ...
                  <span className="text-xs font-mono text-text-data-value">
                    {formatISK(totalJobCost)}
                  </span>
                </div>
                <table className="w-full text-xs">
                  <thead>
                    <tr className="border-b border-overlay-subtle">
                      <th className="px-3 py-1 text-left text-text-muted font-normal">Step</th>
                      <th className="px-2 py-1 text-right text-text-muted font-normal">Qty</th>
                      <th className="px-2 py-1 text-right text-text-muted font-normal">Runs</th>
                      <th className="px-2 py-1 text-right text-text-muted font-normal">Job Fee</th>
                    </tr>
                  </thead>
                  <tbody>
                    {buildSteps.map((step, i) => (
                      <tr key={`${step.type_id}-${i}`} className="border-b border-overlay-subtle/50 hover:bg-interactive-hover">
                        <td className="px-3 py-1 text-text-primary truncate max-w-[180px]">{step.name}</td>
                        <td className="px-2 py-1 text-right font-mono text-text-muted">{Number(step.quantity).toLocaleString()}</td>
                        <td className="px-2 py-1 text-right font-mono text-text-muted">{Number(step.runs ?? 0).toLocaleString()}</td>
                        <td className="px-2 py-1 text-right font-mono text-text-data-value">{formatISK(step.job_cost ?? 0)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <div className="flex items-center justify-center h-full">
                <p className="text-xs text-text-muted">
                  {selectedOpp ? "No build steps" : "Select an opportunity to see job costs"}
                </p>
              </div>
            )}
          </TabsContent>
