...
export default async function handler(
  ...
  try {
    if (req.method === "PUT") {
      const response = await fetch(`${backend}v1/transport/profiles/${id}`, {
        method: "PUT",
    ...
    console.error("Transport profile API error:", error);
    return res.status(500).json({ error: "Failed to process transport profile request" });
  }
}
