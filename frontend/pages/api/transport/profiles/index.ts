...
export default async function handler(
  ...
  try {
    if (req.method === "GET") {
      const response = await fetch(`${backend}v1/transport/profiles`, {
        method: "GET",
    ...
    console.error("Transport profiles API error:", error);
    return res.status(500).json({ error: "Failed to process transport profiles request" });
  }
}
