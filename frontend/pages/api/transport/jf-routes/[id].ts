...
export default async function handler(
  ...
  try {
    if (req.method === "PUT") {
      const response = await fetch(`${backend}v1/transport/jf-routes/${id}`, {
        method: "PUT",
    ...
    console.error("JF route API error:", error);
    return res.status(500).json({ error: "Failed to process JF route request" });
  }
}
