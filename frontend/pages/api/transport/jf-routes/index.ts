...
export default async function handler(
  ...
  try {
    if (req.method === "GET") {
      const response = await fetch(`${backend}v1/transport/jf-routes`, {
        method: "GET",
    ...
    console.error("JF routes API error:", error);
    return res.status(500).json({ error: "Failed to process JF routes request" });
  }
}
