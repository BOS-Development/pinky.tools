...
export default async function handler(
  ...
  try {
    if (req.method === "GET") {
      const response = await fetch(`${backend}v1/transport/trigger-config`, {
        method: "GET",
    ...
    console.error("Transport trigger config API error:", error);
    return res.status(500).json({ error: "Failed to process trigger config request" });
  }
}
