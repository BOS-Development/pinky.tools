...
export default async function handler(
  ...
  try {
    const response = await fetch(`${backend}v1/esi/refresh`, {
      method: "POST",
      headers: {
    ...
    console.error("ESI refresh API error:", error);
    return res.status(500).json({ error: "Failed to trigger ESI refresh" });
  }
}
