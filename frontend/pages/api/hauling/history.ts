...
export default async function handler(
  ...
  try {
    ...
    const response = await fetch(
      `${backend}v1/hauling/history?${params.toString()}`,
      {
        method: "GET",
    ...
    console.error("Hauling history API error:", error);
    return res.status(500).json({ error: "Failed to fetch hauling history" });
  }
}
