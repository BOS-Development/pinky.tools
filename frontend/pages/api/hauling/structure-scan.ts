...
export default async function handler(
  ...
  try {
    const response = await fetch(`${backend}v1/hauling/structures/${id}/scan`, {
      method: "POST",
    ...
    console.error("Hauling structure scan API error:", error);
    return res.status(500).json({ error: "Failed to scan structure market" });
  }
}
