...
export default async function handler(
  ...
  try {
    if (req.method === "GET") {
      const response = await fetch(`${backend}v1/hauling/runs/${id}`, {
        method: "GET",
    ...
    console.error("Hauling run API error:", error);
    return res.status(500).json({ error: "Failed to process hauling run request" });
  }
}
