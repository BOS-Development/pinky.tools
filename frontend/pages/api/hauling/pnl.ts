...
export default async function handler(
  ...
  try {
    if (req.method === "GET") {
      const response = await fetch(`${backend}v1/hauling/runs/${runId}/pnl`, {
        method: "GET",
    ...
    console.error("Hauling P&L API error:", error);
    return res.status(500).json({ error: "Failed to process hauling P&L request" });
  }
}
