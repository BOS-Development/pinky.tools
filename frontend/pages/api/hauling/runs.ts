...
export default async function handler(
  ...
  try {
    if (req.method === "GET") {
      const response = await fetch(`${backend}v1/hauling/runs`, {
        method: "GET",
    ...
    console.error("Hauling runs API error:", error);
    return res.status(500).json({ error: "Failed to process hauling runs request" });
  }
}
