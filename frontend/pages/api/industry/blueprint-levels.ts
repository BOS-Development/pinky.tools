...
export default async function handler(
  ...
  try {
    const response = await fetch(`${backend}v1/industry/blueprint-levels`, {
      method: "POST",
    ...
    console.error("Industry blueprint-levels API error:", error);
    return res.status(500).json({ error: "Failed to fetch blueprint levels" });
  }
}
