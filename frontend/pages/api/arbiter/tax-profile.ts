...
export default async function handler(
  ...
  try {
    const response = await fetch(`${backend}v1/arbiter/tax-profile`, {
      method: req.method,
      headers: {
    ...
    console.error("Arbiter tax profile API error:", error);
    return res.status(500).json({ error: "Failed to fetch arbiter tax profile" });
  }
}
