...
export default async function handler(
  ...
  try {
    const response = await fetch(`${backend}v1/arbiter/settings`, {
      method: req.method,
      headers: {
    ...
    console.error("Arbiter settings API error:", error);
    return res.status(500).json({ error: "Failed to fetch arbiter settings" });
  }
}
