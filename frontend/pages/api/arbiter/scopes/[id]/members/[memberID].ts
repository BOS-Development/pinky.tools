...
export default async function handler(
  ...
  try {
    const response = await fetch(
      `${backend}v1/arbiter/scopes/${id}/members/${memberID}`,
      {
        method: "DELETE",
        headers: {
    ...
    console.error("Arbiter scope member delete API error:", error);
    return res.status(500).json({ error: "Failed to remove scope member" });
  }
}
