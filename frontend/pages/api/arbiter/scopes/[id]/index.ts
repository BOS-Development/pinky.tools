...
export default async function handler(
  ...
  try {
    const response = await fetch(`${backend}v1/arbiter/scopes/${id}`, {
      method: req.method,
      headers: {
    ...
    console.error("Arbiter scope API error:", error);
    return res.status(500).json({ error: "Failed to update arbiter scope" });
  }
}
